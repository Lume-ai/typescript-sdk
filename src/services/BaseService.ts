...
const PROD_ENDPOINT = "https://prod.lume-terminus.com/crud/";
/**
...
export class BaseService {
  ...
  protected async fetchPaginatedData<T>(
    ...
    const response = await this.httpClient.get<PaginatedResponse<T>>(endpoint, {
      params: { page, size },
    });
    return response.data;
  }

  protected async fetchPaginatedDataWithParams<T>(
    endpoint: string,
  ...
  protected async postPaginatedDataWithParams<T>(
    endpoint: string,
    ...
    const response = await this.httpClient.post<PaginatedResponse<T>>(
      endpoint,
      body,
      config
    );
